...
onlyfiles = [ f for f in listdir(input_directory) if (isfile(join(input_directory,f)) and ".cb.xml" in f) ]
...
for files in onlyfiles:
    ...
    #print soup.quote
    #print "SONSONODNOANSODNOASNDOASNDOKNASKODNKOASNDOKASNDKONASKODNOKASNDNOAKSNDKOSANDOKASNDOASND"
    if soup.quote:
        st = soup.quote.extract()

    invalid_tags = ['post', 'img', 'a', 'html', 'body', 'b', 'i']
    for tag in invalid_tags:
        for match in soup.findAll(tag):
            match.replaceWithChildren()

#paragraph = unicode(soup.get_text())
    para_encoded = str(soup).decode('utf-8')
#para_encoded = map(unicode(paragraph))
    ...
    outfilename = join(input_directory, files.replace(".cb.xml", ""))+ ".toAnno.txt"
